    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Get the preview content area where the markdown will be rendered
        var previewContentsElt = document.getElementById('preview-contents');
    
        // Function to replace \begin{} and \end{} blocks with custom divs
        function convertLatexEnvironments(text) {
            return text.replace(/\\begin{(\w+)}([\s\S]*?)\\end{\1}/g, function (wholeMatch, envName, content) {
                if (!environmentMap[envName]) {
                    console.error(`Unknown environment '${envName}'`);
                    return wholeMatch;
                }
                var environment = environmentMap[envName];
                var title = environment.title;
                ...
                return `<div class="latex_${envName}">
                            <span class="latex_title">${title}.</span>
                            ${content}
                        </div>`;
            });
        }
    
        // Hook into markdown conversion process
        var markdownText = document.getElementById('markdown-content').innerHTML;
        var convertedHTML = convertLatexEnvironments(markdownText);
        previewContentsElt.innerHTML = convertedHTML;
    });
